...
def main(args):
    ...
    data = HeartDataModule('train', args.batch_size, args.num_workers, args.data_sink)
    data.setup()
    if args.heads > 1:
        logging.info('Using Ensemble of Decoders...')
        heads = [classifier_head(args.latent * data.classes, LINEAR, n_class=data.classes) for i in range(args.heads)]
        model = EnsembleClassifier(heads, data.features, ENC, args.heads, args.epsilon, args.latent, data.classes)
    else:
        logging.info('Using Sequential Model...')
        head = classifier_head(args.latent * data.classes, LINEAR, n_class=data.classes)
        model = Classifier(head, data.features, ENC, args.epsilon, args.latent, data.classes)

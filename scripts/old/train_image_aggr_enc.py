...
from aggrVAE.image_old.encoder.seq import EncoderClassifier
from aggrVAE.image_old.encoder.ensemble import EnsembleEncoderClassifier
...
datasets = {
    'MNIST' : AggrMNISTDataModule,
    'CIFAR10' : AggrCIFAR10DataModule
}
...
def main(args):
    ...
    data = datasets[args.ds](os.path.join(args.data_source, data_name),args.batch_size, args.num_workers, args.data_sink)

    if args.heads > 1:
        model = EnsembleEncoderClassifier(LINEAR, args.heads, 512, args.latent, data.classes, data.channels)
    else:
        model = EncoderClassifier(LINEAR, 512, args.latent, data.classes, data.channels)
